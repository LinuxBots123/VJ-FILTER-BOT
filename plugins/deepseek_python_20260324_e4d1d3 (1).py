...
from plugins.spell_check import get_spell_suggestions, show_spell_suggestions   # <-- NEW IMPORT
...
BUTTONS1 = {}
BUTTONS2 = {}
# SPELL_CHECK = {}   # <-- REMOVED – now imported from utils
        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ...
                    if total_results > 0:
                        # Results found → proceed normally
                        ai_search = True
                        reply_msg = await message.reply_text(f"<b><i>Searching For {message.text} 🔍</i></b>")
                        await auto_filter(client, message.text, message, reply_msg, ai_search)
                    else:
                        # No results → try spell check
                        suggestions = get_spell_suggestions(search)
                        if suggestions:
                            # Store suggestions in global dict
                            SPELL_CHECK[message.id] = suggestions
                            await show_spell_suggestions(client, message, message.from_user.id, suggestions)
                        else:
                            # No suggestions either, fallback to original no-result message
                            await message.reply_text("No results found. Please check spelling or use /request.")
                    # --- SPELL CHECK INTEGRATION END ---
            except KeyError:
                grpid = await active_connection(str(message.from_user.id))
                await save_group_settings(grpid, 'auto_ffilter', True)
                settings = await get_settings(message.chat.id)
                if settings['auto_ffilter']:
                    # --- SPELL CHECK INTEGRATION (same as above) ---
                    search = message.text
                    files, offset, total_results = await get_search_results(
                        chat_id=message.chat.id,
                        query=search.lower(),
      ...
      #      return
        ...
        async with lock:
            try:
                ...
                logger.exception(e)
        await query.message.edit_text(f"<b>Deleted {deleted} files for {keyword}.</b>")