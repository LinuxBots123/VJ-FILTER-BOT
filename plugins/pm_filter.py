...
async def safe_answer(query, text=None, alert=False):
    await safe_answer(query) if "query" in locals() else None
    try:
        await safe_answer(query, text, show_alert=alert)
    except:
        pass
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not settings["button"]:
        ...
        try:
            await safe_edit(query, reply_markup=reply_markup=InlineKeyboardMarkup(btn))
        except MessageNotModified:
            pass
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if not free_trial_status:            
            ...
            await safe_edit(query, text=text=new_text)
            return
        else:
