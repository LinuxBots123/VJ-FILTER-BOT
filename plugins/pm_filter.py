...
async def safe_answer(query, text=None, alert=False):
    try:
        if text:
            ...
            await safe_answer(query, None, True)
    except:
        pass
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
            ...
            await safe_answer(query, "Yᴏᴜ ɴᴇᴇᴅ ᴛᴏ ʙᴇ Gʀᴏᴜᴘ Oᴡɴᴇʀ ᴏʀ ᴀɴ Aᴜᴛʜ Usᴇʀ ᴛᴏ ᴅᴏ ᴛʜᴀᴛ!", True)
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
                ...
                await safe_answer(query, "Tʜᴀᴛ's ɴᴏᴛ ғᴏʀ ʏᴏᴜ!!", True)
        ...
        if alerts is not None:
            ...
            alert = alerts[int(i)]
            alert = alert.replace("\\n", "\n").replace("\\t", "\t")
            await safe_answer(query, alert, True)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    ...
                    await safe_answer(query, f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", True)
            ...
            await safe_answer(query, 'Uɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ ᴍᴀʜɴ !', True)
        ...
        if userid == 0:
            await safe_answer(query, "You are anonymous admin !", True)
            return
        try:
            btn = await pub_is_subscribed(client, query, settings['fsub'])
            if btn:
                await safe_answer(query, "Kindly Join Given Channel Then Click On Unmute Button", True)
            else:
                await client.unban_chat_member(query.message.chat.id, user_id)
                await safe_answer(query, "Unmuted Successfully !", True)
                try:
                    await query.message.delete()
                except:
                    return
        except:
            await safe_answer(query, "Not For Your My Dear", True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await safe_answer(query, "Jᴏɪɴ ᴏᴜʀ Bᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ ᴍᴀʜɴ! 😒", True)
            return
     ...
     #   if not search:
      ...
      #      return
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
        await safe_answer(query, f"Hey {query.from_user.first_name}, All files on this page has been sent successfully to your PM !", True)
     ...
     #   if not search:
      ...
      #      return
        ...
        if (
            ...
            await safe_answer(query, "Yᴏᴜ Dᴏɴ'ᴛ Hᴀᴠᴇ Tʜᴇ Rɪɢʜᴛs Tᴏ Dᴏ Tʜɪs !", True)
            return
        title = query.message.chat.title
        ...
        if query.from_user.id in ADMINS:
            ...
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", True)
        ...
        if query.from_user.id in ADMINS:
            ...
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", True)
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ !", True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", True)
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uᴘʟᴏᴀᴅᴇᴅ !", True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", True)
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uɴᴀᴠᴀɪʟᴀʙʟᴇ !", True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", True)
        ...
        await safe_answer(query, text=script.REQINFO, True)

    elif query.data == "select":
        await safe_answer(query, text=script.SELECT, True)

    elif query.data == "sinfo":
        await safe_answer(query, text=script.SINFO, True)
        ...
        if status == "True":
            ...
            if set_type == "is_shortlink" and not settings['shortlink']:
                return await safe_answer(query, text = "First Add Your Shortlink Url And Api By /shortlink Command, Then Turn Me On.", True)
