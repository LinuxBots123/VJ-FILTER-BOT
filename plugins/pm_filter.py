...
async def safe_answer(query, text=None, alert=False):
    try:
        if text:
            await safe_answer(query, text, show_alert=alert, True)
        else:
            await safe_answer(query, , True)
    except:
        pass
...
def normalize_query(search):
    if not search:
        return search
    search = search.lower()
    search = re.sub(r'\bs(\d{1})\b', r's0\1', search)
    search = re.sub(r'\be(\d{1})\b', r'e0\1', search)
    return search
...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await safe_answer(query, script.ALRT_TXT.format(query.from_user.first_name, True), show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
   ...
   # if not search:
      #  await safe_answer(query, script.OLD_ALRT_TXT.format(query.from_user.first_name, True),show_alert=True)
       # return
...
async def advantage_spoll_choker(bot, query):
  ...
  #  if not movies:
     #   return await safe_answer(query, script.OLD_ALRT_TXT.format(query.from_user.first_name, True), show_alert=True)
    ...
    await safe_answer(query, script.TOP_ALRT_MSG, True)
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ↭", callback_data=f"next_{req}_{key}_0")])
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        await safe_answer(query, "🚫 𝗡𝗼 𝗙𝗶𝗹𝗲 𝗪𝗲𝗿𝗲 𝗙𝗼𝘂𝗻𝗱 🚫", show_alert=1, True)
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    seas = seas.lower()
    match = re.search(r'(\d+)', seas)
    if match:
        num = int(match.group(1))
        s_format = f"s{num:02d}"
    else:
        s_format = seas

    search = f"{search} {s_format}"
    files, offset, total_results = await get_search_results(chat_id, search, offset=0, filter=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        await safe_answer(query, "Done !", True)
        return
    elif query.data == "gfiltersdeleteallcancel": 
        await query.message.reply_to_message.delete()
        await query.message.delete()
        await safe_answer(query, "Process Cancelled !", True)
        return
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await safe_answer(query, MSG_ALRT, True)
            else:
        ...
        if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
            ...
            await safe_answer(query, "Yᴏᴜ ɴᴇᴇᴅ ᴛᴏ ʙᴇ Gʀᴏᴜᴘ Oᴡɴᴇʀ ᴏʀ ᴀɴ Aᴜᴛʜ Usᴇʀ ᴛᴏ ᴅᴏ ᴛʜᴀᴛ!", show_alert=True, True)
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
                ...
                await safe_answer(query, "Tʜᴀᴛ's ɴᴏᴛ ғᴏʀ ʏᴏᴜ!!", show_alert=True, True)
        ...
        if alerts is not None:
            ...
            alert = alerts[int(i)]
            alert = alert.replace("\\n", "\n").replace("\\t", "\t")
            await safe_answer(query, alert, show_alert=True, True)
    ...
    if query.data.startswith("file"):
        ...
        if not files_:
            return await safe_answer(query, 'Nᴏ sᴜᴄʜ ғɪʟᴇ ᴇxɪsᴛ.', True)
        files = files_
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    ...
                    await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start=short_{file_id}", True)
                    return
                else:
                    await safe_answer(query, f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True, True)
            elif settings['is_shortlink'] and await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start={ident}_{file_id}", True)
                    return
                else:
                    await safe_answer(query, f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True, True)
            ...
            await safe_answer(query, 'Uɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ ᴍᴀʜɴ !', show_alert=True, True)
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles1_{key}", True)
            elif settings['is_shortlink'] and await db.has_premium_access(query.from_user.id):
                await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start={pre}_{key}", True)
                return 
            else:
                await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start={pre}_{key}", True)
            ...
            await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles3_{key}", True)
        except Exception as e:
            logger.exception(e)
            await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles4_{key}", True)
        ...
        if userid == 0:
            await safe_answer(query, "You are anonymous admin !", show_alert=True, True)
            return
        try:
            btn = await pub_is_subscribed(client, query, settings['fsub'])
            if btn:
                await safe_answer(query, "Kindly Join Given Channel Then Click On Unmute Button", show_alert=True, True)
            else:
                await client.unban_chat_member(query.message.chat.id, user_id)
                await safe_answer(query, "Unmuted Successfully !", show_alert=True, True)
                try:
                    await query.message.delete()
                except:
                    return
        except:
            await safe_answer(query, "Not For Your My Dear", show_alert=True, True)
        ...
        if f_caption is None:
            f_caption = f"{files['file_name']}"
        await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}", True)
    
    elif query.data.startswith("checksub"):
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await safe_answer(query, "Jᴏɪɴ ᴏᴜʀ Bᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ ᴍᴀʜɴ! 😒", show_alert=True, True)
            return
        ident, kk, file_id = query.data.split("#")
        await safe_answer(query, url=f"https://t.me/{temp.U_NAME}?start={kk}_{file_id}", True)
     ...
     #   if not search:
      ...
      #      return
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
        await safe_answer(query, f"Hey {query.from_user.first_name}, All files on this page has been sent successfully to your PM !", show_alert=True, True)
     ...
     #   if not search:
      ...
      #      return
        ...
        if (
            ...
            await safe_answer(query, "Yᴏᴜ Dᴏɴ'ᴛ Hᴀᴠᴇ Tʜᴇ Rɪɢʜᴛs Tᴏ Dᴏ Tʜɪs !", show_alert=True, True)
            return
        title = query.message.chat.title
        ...
        if query.from_user.id in ADMINS:
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Hᴇʀᴇ ᴀʀᴇ ᴛʜᴇ ᴏᴘᴛɪᴏɴs !", True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True, True)
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Sᴇᴛ ᴛᴏ Uɴᴀᴠᴀɪʟᴀʙʟᴇ !", True)
            try:
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.message.edit_text(f"<b><strike>{content}</strike></b>")
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Sᴇᴛ ᴛᴏ Uᴘʟᴏᴀᴅᴇᴅ !", True)
            try:
            ...
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True, True)
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.message.edit_text(f"<b><strike>{content}</strike></b>")
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Sᴇᴛ ᴛᴏ Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ !", True)
            try:
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ !", show_alert=True, True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True, True)
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uᴘʟᴏᴀᴅᴇᴅ !", show_alert=True, True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True, True)
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uɴᴀᴠᴀɪʟᴀʙʟᴇ !", show_alert=True, True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True, True)
        ...
        try:
            ...
            print(e)
            await safe_answer(query, f"something went wrong\n\n{e}", show_alert=True, True)
            return
    
    elif query.data == "reqinfo":
        await safe_answer(query, text=script.REQINFO, show_alert=True, True)

    elif query.data == "select":
        await safe_answer(query, text=script.SELECT, show_alert=True, True)

    elif query.data == "sinfo":
        await safe_answer(query, text=script.SINFO, show_alert=True, True)
        ...
        await safe_answer(query, MSG_ALRT, True)
        ...
        await safe_answer(query, "Fetching MongoDb DataBase", True)
        buttons = [[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('⟲ Rᴇғʀᴇsʜ', callback_data='rfrsh')
        ]]
        ...
        if status == "True":
            ...
            if set_type == "is_shortlink" and not settings['shortlink']:
                return await safe_answer(query, text = "First Add Your Shortlink Url And Api By /shortlink Command, Then Turn Me On.", show_alert = True, True)
