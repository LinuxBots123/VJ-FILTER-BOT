...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    ...
                    await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start=short_{file_id}", True)
                    return
                else:
                ...
                if clicked == typed:
                    await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start={ident}_{file_id}", True)
                    return
                else:
                    await safe_answer(query, f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", True)
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start=sendfiles1_{key}", True)
            elif settings['is_shortlink'] and await db.has_premium_access(query.from_user.id):
                await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start={pre}_{key}", True)
                return 
            else:
                await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start={pre}_{key}", True)
            ...
            await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start=sendfiles3_{key}", True)
        except Exception as e:
            logger.exception(e)
            await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start=sendfiles4_{key}", True)
        ...
        if f_caption is None:
            f_caption = f"{files['file_name']}"
        await safe_answer(query, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}", True)
        ...
        await safe_answer(query, f"https://t.me/{temp.U_NAME}?start={kk}_{file_id}", True)
     ...
     #   if not search:
      ...
      #      return
        ...
        await safe_answer(query, script.REQINFO, True)

    elif query.data == "select":
        await safe_answer(query, script.SELECT, True)

    elif query.data == "sinfo":
        await safe_answer(query, script.SINFO, True)
        ...
        if status == "True":
            ...
            if set_type == "is_shortlink" and not settings['shortlink']:
                return await safe_answer(query, "First Add Your Shortlink Url And Api By /shortlink Command, Then Turn Me On.", True)
