...
async def advantage_spoll_choker(bot, query):
  ...
  #  if not movies:
     #   return await safe_answer(query, script.OLD_ALRT_TXT.format(query.from_user.first_name), show_alert=True)
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    search = FRESH.get(key)
    # ❌ Removed space-to-underscore conversion
    # try:
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    for i in range(0, len(QUALITIES)-1, 2):
        btn.append([
            InlineKeyboardButton(
                ...
                callback_data=f"qfl#{QUALITIES[i].lower()}#{key}"
            ),
            InlineKeyboardButton(
                text=QUALITIES[i+1].title(),
                callback_data=f"qfl#{QUALITIES[i+1].lower()}#{key}"
            ),
        ])
    ...
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ↭", callback_data=f"qfl#homepage#{key}")])

    await safe_edit(query, reply_markup=InlineKeyboardMarkup(btn))

@Client.on_callback_query(filters.regex(r"^qfl#"))
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    # ========== FIX: Use word-boundary regex to remove existing quality ==========
    if qual != "homepage":
    ...
    # Update FRESH and BUTTONS with the new search string
    FRESH[key] = search
    BUTTONS[key] = search
    ...
    if query.data.startswith("file"):
     ...
     #   if not search:
      ...
      #      return
        ...
        #await safe_edit(query, text=f"<b>Fetching Files for your query {keyword} on DB... Please wait...</b>")
