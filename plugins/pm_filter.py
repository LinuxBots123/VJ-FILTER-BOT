...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    for i in range(0, len(QUALITIES)-1, 2):
        btn.append([
            InlineKeyboardButton(
                ...
                callback_data=f"qfl#{QUALITIES[i].lower()}#{key}"          # changed to qfl#
            ),
            InlineKeyboardButton(
                text=QUALITIES[i+1].title(),
                callback_data=f"qfl#{QUALITIES[i+1].lower()}#{key}"        # changed to qfl#
            ),
        ])
    ...
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ↭", callback_data=f"qfl#homepage#{key}")])   # changed to qfl#

    await query.edit_message_reply_markup(InlineKeyboardMarkup(btn))

@Client.on_callback_query(filters.regex(r"^qfl#"))   # new prefix for qualities
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    _, qual, key = query.data.split("#")          # variable fixed to qual
    curr_time = datetime.now(pytz.timezone('Asia/Kolkata')).time()   # added missing curr_time
    search = FRESH.get(key)
    try:
        search = search.replace(' ', '_')
    except:
    ...
    try:
        ...
        pass
    searchagain = search
    if qual != "homepage":      # fixed from lang to qual
