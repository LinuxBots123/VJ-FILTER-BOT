...
from utils import get_size, is_subscribed, pub_is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all, get_cap, SPELL_CHECK
...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await safe_answer(query, script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    files, n_offset, total = await get_search_results(chat_id=query.message.chat.id, query=search, offset=offset, filter=True)
    try:
...
async def advantage_spoll_choker(bot, query):
    ...
    await safe_answer(query, script.TOP_ALRT_MSG)
    gl = await global_filters(bot, query.message, text=movie)
    if gl == False:
        k = await manual_filters(bot, query.message, text=movie)
        if k == False:
            files, offset, total_results = await get_search_results(chat_id=query.message.chat.id, query=movie, offset=0, filter=True)
            if files:
                k = (movie, files, offset, total_results)
                ai_search = True
                reply_msg = await safe_edit(query, text=f"<b><i>Searching For {movie} 🔍</i></b>")
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    files, offset, total_results = await get_search_results(chat_id=chat_id, query=search, offset=0, filter=True)
    if not files:
        await safe_answer(query, "🚫 𝗡𝗼 𝗙𝗶𝗹𝗲 𝗪𝗲𝗿𝗲 𝗙𝗼𝘂𝗻𝗱 🚫", True)
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    files, _, _ = await get_search_results(chat_id=chat_id, query=search, max_results=10)
    files = [file for file in files if re.search(seas, file["file_name"], re.IGNORECASE)]
    
    seas1 = "s01" if seas == "season 1" else "s02" if seas == "season 2" else "s03" if seas == "season 3" else "s04" if seas == "season 4" else "s05" if seas == "season 5" else "s06" if seas == "season 6" else "s07" if seas == "season 7" else "s08" if seas == "season 8" else "s09" if seas == "season 9" else "s10" if seas == "season 10" else ""
    search1 = f"{search1} {seas1}"
    BUTTONS1[key] = search1
    files1, _, _ = await get_search_results(chat_id=chat_id, query=search1, max_results=10)
    ...
    files2, _, _ = await get_search_results(chat_id=chat_id, query=search2, max_results=10)
    ...
    if not settings["button"]:
        ...
        try:
            await safe_edit(query, reply_markup=InlineKeyboardMarkup(btn))
        except MessageNotModified:
            pass

# --------------------------- FIXED QUALITIES HANDLER ---------------------------
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    for i in range(0, len(QUALITIES)-1, 2):
        btn.append([
            InlineKeyboardButton(
                ...
                callback_data=f"qf#{QUALITIES[i].lower()}#{key}"
            ),
            InlineKeyboardButton(
                text=QUALITIES[i+1].title(),
                callback_data=f"qf#{QUALITIES[i+1].lower()}#{key}"
            ),
        ])
    ...
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ↭", callback_data=f"qf#homepage#{key}")])
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    # Remove the quality if it's already present using word boundaries
    if qual != "homepage":
        # Use regex with word boundaries to avoid partial matches
        search = re.sub(rf'\b{re.escape(qual)}\b', '', search, flags=re.I).strip()
        # Add the selected quality
        search = f"{search} {qual}".strip()

    # Update FRESH with the new search string
    FRESH[key] = search
    BUTTONS[key] = search
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if not free_trial_status:            
            ...
            await safe_edit(query, text=new_text)
            return
        else:
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    await safe_edit(query, text="Mᴀᴋᴇ sᴜʀᴇ I'ᴍ ᴘʀᴇsᴇɴᴛ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ!!", quote=True)
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
                ...
                await safe_answer(query, "Tʜᴀᴛ's ɴᴏᴛ ғᴏʀ ʏᴏᴜ!!", show_alert=True)
        ...
        if mkact:
            ...
            await safe_edit(query, text='Sᴏᴍᴇ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ!!', parse_mode=enums.ParseMode.MARKDOWN)
        return await safe_answer(query, MSG_ALRT)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    ...
                    await safe_answer(query, f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                ...
                await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start={pre}_{key}")
                return 
            else:
                await safe_answer(query, url=f"https://telegram.me/{temp.U_NAME}?start={pre}_{key}")
        ...
        if userid == 0:
            await safe_answer(query, "You are anonymous admin !", show_alert=True)
            return
        try:
            btn = await pub_is_subscribed(client, query, settings['fsub'])
            if btn:
                await safe_answer(query, "Kindly Join Given Channel Then Click On Unmute Button", show_alert=True)
            else:
                await client.unban_chat_member(query.message.chat.id, user_id)
                await safe_answer(query, "Unmuted Successfully !", show_alert=True)
                try:
                    await query.message.delete()
                except:
                    return
        except:
            await safe_answer(query, "Not For Your My Dear", show_alert=True)
        ...
        await safe_answer(query, url=f"https://t.me/{temp.U_NAME}?start={kk}_{file_id}")
        ...
        files, n_offset, total = await get_search_results(chat_id=query.message.chat.id, query=search, offset=int(offset), filter=True)
        ...
        await safe_edit(query, text="<b>File deletion process will start in 5 seconds !</b>")
        ...
        async with lock:
            try:
                for file in files:
                    ...
                    deleted += 1
                    if deleted % 50 == 0:
                        await safe_edit(query, text=f"<b>Process started for deleting files from DB. Successfully deleted {str(deleted)} files from DB for your query {keyword} !\n\nPlease wait...</b>")
            except Exception as e:
                logger.exception(e)
                await safe_edit(query, text=f'Error: {e}')
            else:
                await safe_edit(query, text=f"<b>Process Completed for file deletion !\n\nSuccessfully deleted {str(deleted)} files from database for your query {keyword}.</b>")
        ...
        await safe_edit(query, text=f"<b>Yᴏᴜʀ sᴇᴛᴛɪɴɢs ᴍᴇɴᴜ ғᴏʀ {title} ʜᴀs ʙᴇᴇɴ sᴇɴᴛ ᴛᴏ ʏᴏᴜʀ PM</b>")
        await query.message.edit_reply_markup(reply_markup)
        if settings is not None:
            buttons = [
                [
        ...
        if query.from_user.id in ADMINS:
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Hᴇʀᴇ ᴀʀᴇ ᴛʜᴇ ᴏᴘᴛɪᴏɴs !")
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        if query.from_user.id in ADMINS:
            ...
            await safe_edit(query, text=f"<b><strike>{content}</strike></b>")
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Sᴇᴛ ᴛᴏ Uɴᴀᴠᴀɪʟᴀʙʟᴇ !")
            try:
        ...
        if query.from_user.id in ADMINS:
            ...
            await safe_edit(query, text=f"<b><strike>{content}</strike></b>")
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Sᴇᴛ ᴛᴏ Uᴘʟᴏᴀᴅᴇᴅ !")
            try:
        ...
        if query.from_user.id in ADMINS:
            ...
            await safe_edit(query, text=f"<b><strike>{content}</strike></b>")
            await query.message.edit_reply_markup(reply_markup)
            await safe_answer(query, "Sᴇᴛ ᴛᴏ Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ !")
            try:
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ !", show_alert=True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uᴘʟᴏᴀᴅᴇᴅ !", show_alert=True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await safe_answer(query, f"Hᴇʏ {user.first_name}, Yᴏᴜʀ Rᴇᴏ̨ᴜᴇsᴛ ɪs Uɴᴀᴠᴀɪʟᴀʙʟᴇ !", show_alert=True)
        else:
            await safe_answer(query, "Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        await safe_answer(query, text=script.REQINFO, show_alert=True)

    elif query.data == "select":
        await safe_answer(query, text=script.SELECT, show_alert=True)

    elif query.data == "sinfo":
        await safe_answer(query, text=script.SINFO, show_alert=True)
        ...
        if status == "True":
            ...
            if set_type == "is_shortlink" and not settings['shortlink']:
                return await safe_answer(query, text = "First Add Your Shortlink Url And Api By /shortlink Command, Then Turn Me On.", show_alert = True)
